...
def main():
    ...
    # UI 側が --inputs_dir だけ渡してくるケースに対応（--input_image が必須だと UI が落ちる）
    ap.add_argument("--input_image", required=False, default=None)
    ap.add_argument(
        "--inputs_dir",
        required=False,
        default=None,
        help="(optional) 入力画像フォルダ。--input_image が無い場合、この中の画像を対象にする。",
    )
    ...
    # ---- input_image 決定（UI が --inputs_dir のみ渡す場合に対応） ----
    if not args.input_image:
        if not args.inputs_dir:
            raise SystemExit(
                "ERROR: --input_image または --inputs_dir を指定してください"
            )

        cand = []
        for ext in (".png", ".jpg", ".jpeg", ".webp"):
            cand.extend(sorted(glob.glob(os.path.join(args.inputs_dir, f"*{ext}"))))
        if not cand:
            raise FileNotFoundError(f"no input images in {args.inputs_dir}")

        # まずは 1 枚目を使う（UI 側で選択 UI があるなら --input_image が渡される想定）
        args.input_image = cand[0]

    # tiles_dir が未指定でも default='tiles'。ただし inputs_dir が指定されている場合は
    # inputs/<basename_without_ext>/ を期待する UI もあるため、存在する方を優先する。
    if args.inputs_dir:
        base = os.path.splitext(os.path.basename(args.input_image))[0]
        cand_tiles = os.path.join(args.tiles_dir, base)
        if os.path.isdir(cand_tiles):
            args.tiles_dir = cand_tiles
